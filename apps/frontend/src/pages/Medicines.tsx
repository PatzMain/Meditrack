...
const Medicines = () => {
  ...
  const [archiveMedicine, setArchiveMedicine] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    if (highlightId && highlightId !== highlightedId) {
      // Check if this is a fresh navigation to the medicines page
      // If user came from universal search, the highlight should be recent
      const searchTimestamp = localStorage.getItem('searchTimestamp');
      const now = Date.now();
      const maxAge = 5000; // 5 seconds max age for search highlights
      
      if (searchTimestamp && (now - parseInt(searchTimestamp)) < maxAge) {
        ...
        // Clear stale search highlights
        console.log('Clearing stale search highlight');
  ...
  }, [highlightedId, activeTab, searchTerm, filters, sortField, sortDirection]); // Depend on variables that affect table rendering
  ...
  // Listen for direct search highlight events (when already on the same page)
  useEffect(() => {
    const handleSearchHighlight = (event: CustomEvent) => {
      console.log('Direct search highlight event received:', event.detail);
      setHighlightedId(event.detail);
    ...
    return () => window.removeEventListener('searchHighlight', handleSearchHighlight as EventListener);
  }, []);
  ...
  useEffect(() => {
    ...
    // Cleanup function to clear stale search highlights when component unmounts
    return () => {
      // Only clear if the search is older than 1 second to avoid clearing during navigation
      const searchTimestamp = localStorage.getItem('searchTimestamp');
      if (searchTimestamp) {
        const age = Date.now() - parseInt(searchTimestamp);
        if (age > 1000) {
          localStorage.removeItem('searchHighlight');
          localStorage.removeItem('searchTimestamp');
        }
  ...
  const currentData = useMemo(() => {
    return medicines.filter(medicine => medicine.type === activeTab);
  }, [medicines, activeTab]);
  ...
  const confirmArchive = async () => {
    if (archiveMedicine) {
      try {
        await medicineApi.archive(archiveMedicine.id);
        
        // Remove from local state
        setMedicines(prev => prev.filter(m => m.id !== archiveMedicine.id));
        
        // Refresh search index
        universalSearch.refreshMedicinesIndex();
        
        // Show success message (you could use a toast notification here)
        console.log(`${archiveMedicine.name} has been archived successfully!`);
      } catch (err: any) {
        console.error('Failed to archive medicine:', err);
        setError(err.response?.data?.error || 'Failed to archive medicine');
      }
    }
  ...
  return (
    <div className="p-6">
      <div className="max-w-7xl mx-auto space-y-8">
              ...
              <div className="flex items-center justify-center space-x-3">
                ...
                <div>
                  ...
                  <div className={`text-sm font-normal ${
                    ...
                    {medicines.filter(m => m.type === 'medical').length} items
                  </div>
                </div>
              </div>
            </button>
              ...
              <div className="flex items-center justify-center space-x-3">
                ...
                <div>
                  ...
                  <div className={`text-sm font-normal ${
                    ...
                    {medicines.filter(m => m.type === 'dental').length} items
                  </div>
                </div>
              </div>
            </button>
            ...
            <table className="w-full table-auto divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {loading ? (
                  ...
                  return (
                  <tr key={medicine.id} data-medicine-id={medicine.id} className={`group hover:bg-gradient-to-r transition-all duration-200 ${
                    activeTab === 'medical' 
            ...
            <div className="bg-white rounded-3xl p-8 max-w-4xl w-full mx-4 shadow-2xl transform transition-all duration-300 max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={async (e) => {
                ...
                try {
                  ...
                  console.log(`${newMedicine.name} has been added successfully!`);
                } catch (err: any) {
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white rounded-3xl p-8 max-w-4xl w-full mx-4 shadow-2xl transform transition-all duration-300 max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={async (e) => {
                ...
                try {
                  const response = await medicineApi.update(editMedicine.id, updatedData);
                  
                  // Update local state
                  setMedicines(prev => prev.map(m => 
                    m.id === editMedicine.id ? response.data.data : m
                  ));
        ...
        {showArchiveModal && archiveMedicine && (
          <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white rounded-3xl p-8 max-w-md w-full mx-4 shadow-2xl transform transition-all duration-300">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600 mb-6">
                  Are you sure you want to archive <span className="font-semibold text-gray-900">"{archiveMedicine.name}"</span>? This action will remove the medicine from the active inventory.
                </p>
                ...
                <div className="bg-gray-50 rounded-xl p-4 mb-6 text-left">
                  <div className="flex items-center space-x-3 mb-3">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900">{archiveMedicine.name}</p>
                      <p className="text-sm text-gray-600">{archiveMedicine.generic_name}</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      <span className="text-gray-600">Stock:</span>
                      <span className="font-medium text-gray-900 ml-1">{archiveMedicine.quantity} units</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Category:</span>
                      <span className="font-medium text-gray-900 ml-1">{archiveMedicine.category}</span>
                    </div>
                  </div>
                </div>
